...
export function AdminRecordForm({ open, onOpenChange, record, onSuccess }: AdminRecordFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    client_bonus: 0,
    // Admin can edit these additional fields
  ...
  useEffect(() => {
    if (record) {
      setFormData({
        ...
        financial_load_percent: record.financial_load_percent || 5,
        manager_bonus_percent: record.manager_bonus_percent || 3,
        nds_tax_rate: 12,
        kpn_tax_rate: 20,
      })
    } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Администратор может редактировать все поля. Желтые поля - основные данные, зеленые - налоговые настройки.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="border-t pt-4">
            <h3 className="text-lg font-semibold mb-4">Административные настройки</h3>
            <div className="grid gap-4 md:grid-cols-4">
              <div className="space-y-2">
                <Label htmlFor="financial_load_percent">Финансовая нагрузка %</Label>
                <Input
                  id="financial_load_percent"
                  type="number"
                  min="0"
                  max="100"
                  step="0.1"
                  value={formData.financial_load_percent}
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, financial_load_percent: Number.parseFloat(e.target.value) || 5 }))
                  }
              ...
              <div className="space-y-2">
                <Label htmlFor="manager_bonus_percent">% менеджера</Label>
                <Input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, manager_bonus_percent: Number.parseFloat(e.target.value) || 3 }))
                  }
                  className="border-green-500"
                />
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="nds_tax_rate">НДС %</Label>
                <Input
                  id="nds_tax_rate"
                  type="number"
                  min="0"
                  max="100"
                  step="0.1"
                  value={formData.nds_tax_rate}
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, nds_tax_rate: Number.parseFloat(e.target.value) || 12 }))
                  }
                  className="border-green-500"
                />
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="kpn_tax_rate">КПН %</Label>
                <Input
                  id="kpn_tax_rate"
                  type="number"
                  min="0"
                  max="100"
                  step="0.1"
                  value={formData.kpn_tax_rate}
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, kpn_tax_rate: Number.parseFloat(e.target.value) || 20 }))
                  }
                  className="border-green-500"
                />
              </div>
